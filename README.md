...
### 8️⃣ MCP 파일시스템 서버 (Secure File Access)
Claude Code가 파일을 직접 조작하는 대신, **MCP(Model Context Protocol) 표준 프로토콜**을 통해 허용된 디렉토리 내에서만 안전하게 파일 작업을 수행할 수 있습니다. 12개의 파일시스템 도구(읽기, 쓰기, 검색, 트리 구조 등)를 제공하며, 경로 검증·심링크 공격 차단·화이트리스트 기반 접근 제어로 보안을 강화합니다. `config.yaml`의 `mcp` 섹션에서 활성화하고 허용 디렉토리를 지정할 수 있습니다.

### 9️⃣ 투명한 가시성 (Real-time Visibility)
...
- [`docs/사용자-가이드.md`](docs/사용자-가이드.md) — 봇 생성, 토큰 발급, 상세 설정(config), 명령어, MCP 설정, 트러블슈팅
- [`docs/아키텍처.md`](docs/아키텍처.md) — 전체 시스템 설계도, 멀티채널 어댑터 계층, MCP 서버 구조, 메시지 라이프사이클
