...
### 5️⃣ 외부 앱 연동 인터페이스 (Embeddable Package)
단독 실행(Standalone)뿐만 아니라, 기존에 운영 중인 다른 애플리케이션(FastAPI, 기타 봇 등)에 라이브러리 형태로 깔끔하게 탑재할 수 있습니다. `on_result` 콜백을 통해 AI의 최종 처리 결과를 받아 DB에 무결하게 저장하거나 후처리 비즈니스 로직을 태울 수 있으며, `ChannelSender`를 통해 오케스트레이터가 가진 강력한 파일 큐 기반 발송 시스템(`enqueue`)을 호스트 앱에서도 그대로 빌려 쓸 수 있습니다.
...
```bash
git clone https://github.com/user/GSD-Orchestrator.git
cd GSD-Orchestrator

# TELEGRAM_BOT_TOKEN 및 SLACK_BOT_TOKEN(선택) 설정
cp .env.example .env          

# 가상환경 생성 및 의존성 설치, 즉시 실행 (슬랙 연동 시 pip install slack_bolt 추가 필요)
./start.sh                    
```
...
A. 네, 여러 개의 오케스트레이터를 띄울 필요가 없습니다. 본 시스템은 팀의 용도와 목적에 맞게 슬랙이나 텔레그램의 **단일 채널들(어댑터)을 개설하여 사용하는 채널 기반 확장 구조**로 설계되었으므로, 오케스트레이터 자체는 단일 인스턴스로 유지하는 것이 관리상 효율적입니다.
...
- [`docs/외부-연동-가이드.md`](docs/외부-연동-가이드.md) — 외부 앱 연동: ChannelSender, on_result 콜백, 설치 및 사용법
